...
imagens = []
for _, _, files in os.walk(os.path.abspath('assets/imagens_originais')):
    for file in files:
        imagens.append(file)
...
for imagem_atual in imagens:
    ...
    if len(placas_detectadas) > 0:
        ...
        # binariza a imagem
        # img_binarizada = Binarizacao(img_plp, 92).executar()
        ...
        # dilatação
        # mg_mat = cv2.dilate(img_mat, elip, iterations=1)
